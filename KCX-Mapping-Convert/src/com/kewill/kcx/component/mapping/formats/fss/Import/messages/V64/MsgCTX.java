...
import com.kewill.kcx.component.mapping.formats.fss.Import.subsets.V64.TsAB1;
import com.kewill.kcx.component.mapping.formats.fss.Import.subsets.V64.TsAB2;
...
import com.kewill.kcx.component.mapping.formats.fss.Import.subsets.V64.TsSUA;
...
public class MsgCTX extends FssMessage{
	...
	private List <TsSUA> suaList;  // SumA 
	...
	public void setSUAList(List <TsSUA> list) {
		suaList = list;
    }
	public List <TsSUA> getSUAList() {
		return suaList;
	}
	public void addSUAList(TsSUA subset) {
		if (suaList == null) {
			suaList = new Vector<TsSUA>();	
		}
		suaList.add(subset);
    }
	...
	public String getFssString() throws FssException {
		...
		if (suaList != null) {                     					 
			int suaSize = suaList.size();
			for (int j = 0; j < suaSize; j++) {
				TsSUA sua = (TsSUA) suaList.get(j);
				if (sua != null && !sua.isEmpty()) {
					res = appendString(res, sua.teilsatzBilden());
				}
			}
		}
		if (unbList != null) {                     					 
        ...
        try {
            ...
            while ((line = message.readLine()) != null) {
                ...
                if (lineType.equalsIgnoreCase("AB1")) {
                    ...
                    if (txaSubset.getTxaart().equals("AAH")) {
                    	if (txaList == null) {
                    		txaList = new Vector<TsTXA>();
                    	}                   
                    ...
                    if (adrSubset.getKztyp().equals("01")) {
                    	anmelder = adrSubset;
                    } else if (adrSubset.getKztyp().equals("02")) {
                    	vertreter = adrSubset;
                    } else if (adrSubset.getKztyp().equals("04")) {
                	...
                	if (txpSubset.getTxpart().equals("AAH") && pos != null) {               		
                		pos.addTXPList(txpSubset);                		                		
                	}
                } else if (lineType.equalsIgnoreCase("AHI")) {  
                	TsAHICTX ahiSubset = new TsAHICTX();    
