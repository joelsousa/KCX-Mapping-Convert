...
public class BodySubmittedDraftOfSplittingOperationUids extends UidsMessageEMCS  {
	...
	private void writeSplitDetailsEAD(SplitDetailsEAD split) throws XMLStreamException {
    	...
    	openElement("SplitDetailsEAD");
	    	...
	    	writeChangedDestination(split, version);	    	
	    	writeTrader("NewTransportArrangerTrader", split.getNewTransportArranger(), version);
	    	writeTrader("NewTransporterTrader", split.getNewTransporter(), version);
	...
	private void writeChangedDestination(SplitDetailsEAD split, String version) throws XMLStreamException {
		if (split == null) {
			return;
		}
